...
Attempt 1 — password change interrupted:
```
Password:
...
Attempt 2 — wrong password on login:
```
Password:
...
user's own primary group. This is exactly the behavior the setgid bit is
meant to produce.
...
Cause: missing space between `su -` and `sade` (`su -sade` was misparsed),
combined with an incorrect password on the first two attempts.
...
(`You must choose a longer password.`). A longer password was accepted on
the second attempt.
...
Successful — no error. Confirms `sade` retains normal access within his
own department while being fully blocked from every other department.
...
2. **`su: Authentication failure`** (mogunleye and sade, multiple times) —
   incorrect password entered; resolved by re-attempting with the correct
   password.
...
were required — they were user-input errors, not permission or setgid
misconfigurations.
...
case (department member, full access) and negative case (outsider, no
access) is what confirms least-privilege was actually achieved, not just
configured.
