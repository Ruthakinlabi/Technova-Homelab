...
5. Wrap the account-creation logic in a function, not inline code, so it can be tested and reused independently (e.g. called once per CSV row, or later reused for a single ad-hoc hire outside a bulk run).
6. Test it against the real 67-row file and confirm every account was created correctly.

### Why a Function, Not Inline Code

Task 1's script did everything inline, line by line, inside the loop. That was fine for validation — read a row, check it, print a result. Provisioning is different: the same exact sequence of steps (generate username, check for conflict, create account, assign group) needs to run once per employee, with the only thing changing being the input. That repetition is precisely what a function is for — write the logic once, call it 67 times, rather than duplicating (or subtly varying) the same block of commands throughout the script.

### Why Duplicate-Username Handling Matters Here Specifically

With 67 real employees and Nigerian naming patterns in this dataset, it's entirely plausible for the first-initial + surname convention to produce a collision — e.g. two different people both reducing to bokoro. A script that assumes every generated username is automatically unique will either crash outright or, worse, silently create a duplicate/conflicting account.
...
Department to group map: declare -A DEPT_TO_GROUP is a Bash associative array that maps each department value from the CSV to its actual Linux group name. This is what satisfies "no hardcoded department names in the account-creation logic itself" — the mapping lives in one lookup table, and create_employee_account just reads from it (${DEPT_TO_GROUP[$department]}) instead of containing if department == "Engineering" style branching. CustomerSupport is kept mixed-case because that's the real group name on the system; everything else happens to be lowercase.

Counters: created_count and skipped_count are plain integer variables, incremented inside the function, and printed in the summary at the end.

generate_username(): implements the first-initial + surname convention. awk '{print $1}' grabs the first word (first name); awk '{print $NF}' grabs the last field regardless of how many words are in between (handles middle names safely). ${first_name:0:1} is Bash substring syntax for "first character." The result is lowercased with tr so "Tolu Bakare" becomes tbakare. All variables are local so this function doesn't leak state into the rest of the script.

create_employee_account() is the core function requested in Responsibility 5. It takes one row's data as three parameters, so it can be called once per CSV line or independently for a single ad-hoc hire. It does three checks in order, each with its own early return:
...
Main loop: tail -n +2 skips the CSV header row. IFS=',' read -r splits each line on commas into the three variables. xargs on each variable trims stray leading/trailing whitespace (common after a comma-split). Each cleaned row is passed straight into create_employee_account.
...
Validation: correct account created for each new row (id and getent passwd confirm correct UID, GID, home directory, and shell), and the pre-existing username (eadeyemi) was correctly skipped rather than causing a crash or duplicate. Test accounts were then removed with userdel -r to reset the environment before the real run.
...
- The collision-check logic (tested and proven in the 3-row test run against a pre-existing eadeyemi account) was in place for this run as well, even though no collision happened to occur in the real data.
